...
export const AddDesignModal: React.FC<AddDesignModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !name.trim()) {
      alert("Please provide Design Name");
      return;
    }
    ...
    try {
      const payload = {
        name: name.trim(),
        modelNumber: modelNumber ? modelNumber.trim().toUpperCase() : "",
        type,
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-stone-700 mb-1">
                Model / Design # <span className="text-[10px] text-stone-400 font-normal">(Optional)</span>
              </label>
              <input
                type="text"
                value={modelNumber}
                onChange={(e) => setModelNumber(e.target.value)}
                placeholder="e.g. AC-EMB-108 (or auto)"
